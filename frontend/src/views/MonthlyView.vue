...
<template>
  <div class="mx-auto max-w-6xl space-y-7">
    ...
    <section v-if="isComposerOpen" class="border-y border-slate-200 bg-white">
      <div class="grid gap-0 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.88fr)]">
        <div class="space-y-4 p-5 lg:border-r lg:border-slate-200">
          ...
          <button class="inline-flex items-center gap-1 text-xs font-medium text-teal-700 hover:text-teal-900" type="button">
            <ImagePlus class="h-3.5 w-3.5" /> 另附佐证材料
          </button>
        </div>

        <div class="space-y-3 p-5">
          <div class="flex items-center justify-between"><span class="field-label">发票</span><span class="text-xs text-slate-500">上传后自动识别</span></div>
    ...
    <section class="grid gap-px overflow-hidden border border-slate-200 bg-slate-200 sm:grid-cols-2">
      <div class="bg-white px-5 py-4">
        <p class="text-xs text-slate-500">本月已提交</p>
        <p class="mt-1 text-xl font-semibold text-ink">{{ formatCurrency(submittedTotal) }} <span class="text-base font-medium text-slate-500">/ {{ submittedRecords.length }} 笔</span></p>
      </div>
      <div class="bg-white px-5 py-4">
        <p class="text-xs text-slate-500">待补材料</p>
        <p class="mt-1 text-xl font-semibold text-amber-700">{{ materialMissingCount }} 笔</p>
      </div>
    </section>
