    ...
    <header class="overflow-hidden rounded-panel border border-accent-line bg-accent-soft p-6 sm:p-8">
      ...
      <div class="mt-6 flex flex-wrap gap-2">
        <button
          ...
          :key="item.id"
          class="chip px-3 py-1.5 transition duration-1 ease-standard hover:border-accent-line hover:bg-accent-soft hover:text-accent-ink"
          type="button"
      ...
      <aside class="hidden lg:block">
        <nav class="sticky top-6 space-y-1 rounded-control border border-hairline bg-white p-3">
          ...
          <button
            v-for="item in sections"
            :key="`nav-${item.id}`"
            class="block w-full rounded-control px-2 py-2 text-left text-sm text-slate-600 transition duration-1 ease-standard hover:bg-surface-soft hover:text-accent-ink"
            type="button"
            @click="scrollTo(item.id)"
          >
            {{ item.label }}
      ...
      <div class="space-y-6">
        ...
        <section id="overview" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <Home class="h-5 w-5 text-accent-ink" />
            <h2>系统简介</h2>
          </div>
          <div class="guide-card">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <div class="rounded-control border border-accent-line bg-accent-soft p-4">
                <p class="text-xs font-semibold text-accent-ink">管理员额外可访问</p>
                <ul class="guide-list mt-2">
        ...
        <section id="login" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <Building2 class="h-5 w-5 text-accent-ink" />
            <h2>登录与账号</h2>
          </div>
          <div class="guide-card space-y-4">
            ...
            <p class="rounded-control border border-state-warn-line bg-state-warn-soft px-3 py-2 text-sm text-state-warn-ink">
        ...
        <section id="submit" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <Upload class="h-5 w-5 text-accent-ink" />
          ...
          <div class="guide-card space-y-5">
            ...
            <div class="rounded-control border border-hairline bg-surface-soft p-4 text-sm text-slate-600">
        ...
        <section id="ledger" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <ReceiptText class="h-5 w-5 text-accent-ink" />
        ...
        <section v-if="isAdmin" id="review" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <CheckCircle2 class="h-5 w-5 text-accent-ink" />
            <h2>审核与预览</h2>
          </div>
          <div class="guide-card space-y-4">
            <ol class="guide-steps">
        ...
        <section v-if="isAdmin" id="users" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <Users class="h-5 w-5 text-accent-ink" />
            <h2>人员管理</h2>
          </div>
          <div class="guide-card space-y-4">
            ...
            <div class="overflow-hidden rounded-control border border-hairline">
              <table class="min-w-full text-left text-sm">
                <thead class="bg-surface-soft text-xs text-slate-500">
                  <tr>
        ...
        <section id="status" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <ClipboardList class="h-5 w-5 text-accent-ink" />
            <h2>记录状态说明</h2>
          </div>
          <div class="guide-card">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-3 rounded-control border border-hairline p-3">
        ...
        <section id="faq" class="guide-section scroll-mt-6">
          <div class="guide-section-head">
            <HelpCircle class="h-5 w-5 text-accent-ink" />
            <h2>常见问题</h2>
          </div>
          <div class="guide-card divide-y divide-hairline">
...
.guide-card {
  @apply rounded-panel border border-hairline bg-white p-5 text-sm leading-relaxed text-slate-700 shadow-raised;
}
...
.guide-inline-icon {
  @apply mt-0.5 h-3.5 w-3.5 shrink-0 text-accent-ink;
}
