...
11. 报销台账：普通员工通过 `/api/ledger` 按年份、月份、类别、状态查询自己的 `expenses`；管理员先通过 `/api/admin/users` 获取当前启用员工的下拉选项，再通过 `/api/admin/ledger` 按月份、公司、员工、类别、状态查询全员记录，员工筛选走稳定的 `employee_id`（`user_id`）而不是姓名，避免 MentiHub 改名或目录名与快照不一致时漏查。两者共用台账查询服务，同时查看交易记录附件和发票匹配摘要。
12. 导出预览：后端合并统计 `matched` 和 `reviewed` 记录，同时返回筛选条件下的 `pending` 数量；前端在独立确认框中以明确月份调用预览，避免误导出全部历史。
13. 单 Excel 导出：`/api/admin/export.xlsx` 保留轻量台账文件。
14. 明细包导出：`POST /api/admin/export-packages` 生成带随机令牌的临时 ZIP，附件按 `{公司}{期间}报销/{员工}{期间}报销/{报销类别}/{发票或佐证文件}` 归档；例如 `山途远智5月报销/夏莺萁05月报销/办公采购/快递发票20元.pdf`。外层压缩包使用真实筛选期间命名，例如 `山途远智-2026年5月-报销明细.zip`。前端通过 `GET /api/admin/export-packages/{id}` 的 HTTP Range 能力并行下载四段、按顺序合并并显示总进度，规避大陆到香港线路的单连接吞吐瓶颈。临时文件仅管理员可访问，下载完成后立即删除，异常遗留文件在一小时后清理。交易记录附件来自 `expense_attachments`，发票文件来自 `expense_invoice_allocations`。
