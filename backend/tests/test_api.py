...
def test_employee_ledger_only_returns_own_records(tmp_path, monkeypatch):
    ...
    dandi_expense = create_invoice_backed_expense(
        client,
        dandi,
        user_id=2,
        project_name="管理员的项目",
        month="2026-06",
        invoice_number="DANDI-PRIVATE",
    )
    ouyang_expense = create_invoice_backed_expense(
        client,
        ouyang,
        user_id=3,
        project_name="员工自己的项目",
        month="2026-06",
        invoice_number="OUYANG-OWN",
    )
    with client.app.state.db.connect() as connection:
        dandi_attachment_id = connection.execute(
            "SELECT attachment_id FROM expense_invoice_allocations WHERE expense_id = ?",
            (dandi_expense["id"],),
        ).fetchone()["attachment_id"]
        ouyang_attachment_id = connection.execute(
            "SELECT attachment_id FROM expense_invoice_allocations WHERE expense_id = ?",
            (ouyang_expense["id"],),
        ).fetchone()["attachment_id"]
        connection.execute(
            "UPDATE attachments SET file_hash = 'shared-private-hash' WHERE id IN (?, ?)",
            (dandi_attachment_id, ouyang_attachment_id),
        )
        connection.execute(
            "UPDATE attachments SET duplicate_count = 1 WHERE id = ?",
            (ouyang_attachment_id,),
        )
        connection.execute(
            "UPDATE expenses SET has_duplicate = 1 WHERE id = ?",
            (ouyang_expense["id"],),
        )
    ...
    assert admin_ledger.json()[0]["duplicate_of"][0]["employee_name"] == "艾丹迪"
